import { addMonths, format, formatISO, parseISO, subDays, subMonths } from "date-fns";
...
export default async function fetchDatabase() {
  ...
  const data =
    (now_showing.length !== 0) & (coming_soon.length !== 0)
      ? { now_showing: now_showing, coming_soon: coming_soon, date_created: timeStamp }
      : null;
